...
@section('content')
@section('title', 'Sector-SF-Li|Lists')
            ...
            <div class="content-header-left col-md-12 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        ...
                        <a href="{{route('SectorSFD.restoreList')}}" style="padding: 20px;">
                            ...
                            <table id="html5-extension" class="table table-bordered non-hover" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center badge rounded-pill badge-light-primary me-1" style="margin-top: 12px; display:flex;justify-content:center;">{{$SectorSFD_DetailsFetch->get_sector_special_feature_section->heading}}</td>
                                        <td class="text-center">{{$SectorSFD_DetailsFetch->featureName}}</td>
                                        <td>
                                            <input type="checkbox" class="toggle-class" data-id="{{ $SectorSFD_DetailsFetch->id }}" data-toggle="toggle" data-style="slow" data-on="Enabled" data-off="Disabled" {{ $SectorSFD_DetailsFetch->status == true ? 'checked' : ''}}>
                                        </td>
                                        <td>
...
<script>
    $('.toggle-class').on('change', function() {
        ...
        var sector_Special_feature_id = $(this).data('id');
        $.ajax({
            type: 'GET'
            , dataType: 'JSON'
            , url: "{{ route('Sector_SFD_li') }}"
            , data: {
                'status': status
                , 'sector_Special_feature_id': sector_Special_feature_id
            }
